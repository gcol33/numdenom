...
## The quotr Solution: Joint Modelling

quotr takes a different approach. Instead of modelling the ratio, we jointly model the two processes that generate it:
...
This is why quotr makes shared structure the **default**. Independence is a special case that must be explicitly requested—and triggers a warning.

## The Three Families

quotr provides three model families for different data types:

### 1. Two-Process Counts: `quotr_negbin_negbin()`
...
### 3. Count/Effort: `quotr_poisson_gamma()`
...
Because ratios are computed in Stan's `generated quantities` block, every posterior draw of the ratio reflects uncertainty in:
...
| **quotr (shared structure)** | Correct propagation of uncertainty, shared drivers cancel |

quotr enforces the correct approach by design:
