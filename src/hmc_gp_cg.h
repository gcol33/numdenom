...
#ifndef RATIOD_HMC_GP_CG_H
#define RATIOD_HMC_GP_CG_H
...
inline int dense_cg_solve(
  ...
  double r_dot_r = ratiod_linalg::dot_product(r.data(), r.data(), k);
  double b_norm = std::sqrt(ratiod_linalg::dot_product(b, b, k));
  ...
  for (int iter = 0; iter < maxiter; iter++) {
    ...
    double p_dot_Cp = ratiod_linalg::dot_product(p.data(), Cp.data(), k);
    ...
    double r_dot_r_new = ratiod_linalg::dot_product(r.data(), r.data(), k);
...
inline int dense_pcg_solve(
  ...
  double r_dot_z = ratiod_linalg::dot_product(r.data(), z.data(), k);
  double b_norm = std::sqrt(ratiod_linalg::dot_product(b, b, k));
  if (b_norm < 1e-14) b_norm = 1.0;
  ...
  for (int iter = 0; iter < maxiter; iter++) {
    ...
    double r_norm = std::sqrt(ratiod_linalg::dot_product(r.data(), r.data(), k));
    if (r_norm / b_norm < tol) {
      return iter + 1;
    }
    ...
    double r_dot_z_new = ratiod_linalg::dot_product(r.data(), z.data(), k);
...
double gp_nngp_log_lik_gpu(
  ...
  if (!batch_obs_indices.empty()) {
    ...
    bool gpu_success = ratiod_gpu::gpu_batched_cholesky_solve(C_batch, c_batch, alpha_batch, nn);
