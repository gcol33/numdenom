// Binomial model with temporal GP - matches numdenom exactly
// Single outcome binomial with temporal GP

data {
  int<lower=1> N;           // Number of observations
  int<lower=1> T;           // Number of unique time points
  array[N] int<lower=0> y;  // Successes (binomial)
...
parameters {
  // Fixed effects (binomial on logit scale)
  real beta_0;          // Intercept
  real beta_1;          // Slope for x

  // GP parameters - BOUNDED for stability
  real<lower=0> sigma_gp;   // GP marginal standard deviation
...
model {
  vector[N] logit_p;   // Binomial logit probability
  ...
  // Likelihood - binomial with GP effect
  for (n in 1:N) {
    logit_p[n] = beta_0 + beta_1 * x[n] + gp_effects[time_idx[n]];
  }

  // Binomial likelihood (logit parameterization)
  y ~ binomial_logit(trials, logit_p);
}
