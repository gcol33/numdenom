// Joint Binomial model with SHARED RE + ICAR + RW1 + Spatiotemporal Type I
// Knorr-Held Type I: Unstructured interaction (IID)
// Row 90 in gradient_methods.md
//
// Model: logit(p) = X*beta + re[g] + phi_s[s] + phi_t[t] + delta[s,t]
// where delta[s,t] ~ N(0, 1/tau_delta) (unstructured)
...
parameters {
  ...
  // SHARED spatiotemporal interaction (Type I: IID)
  matrix[J, T] delta_raw;
  real<lower=0> tau_delta;
}

transformed parameters {
  vector[n_groups] re = sigma_re * z_re;
  matrix[J, T] delta = delta_raw / sqrt(tau_delta);
}
...
model {
  ...
  for (n in 1:N) {
    int s = spatial_idx[n];
    int t = time_idx[n];
    eta[n] += re[group_idx[n]] + phi_spatial[s] + phi_temporal[t] + delta[s, t];
  }
  ...
  // Spatiotemporal interaction priors (Type I: IID)
  tau_delta ~ gamma(1, 0.01);
  to_vector(delta_raw) ~ std_normal();
  sum(to_vector(delta)) ~ normal(0, 0.001 * J * T);
