// Joint Poisson-Gamma model with SHARED random effects + RW2 temporal
// Matches numdenom ratiod_poisson_gamma() with (1|site) + temporal_rw2()
// Row 12 in gradient_methods.md
//
...
// Penalizes curvature rather than first differences
...
data {
  ...
  // Temporal structure
  int<lower=3> T;                      // Number of time points (need T >= 3 for RW2)
...
model {
  ...
  // phi[t] - 2*phi[t-1] + phi[t-2] ~ N(0, 1/sqrt(tau))
  for (t in 3:T) {
