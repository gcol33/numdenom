// Joint Lognormal model with SHARED RE + ICAR spatial
// Row 100 in gradient_methods.md
...
model {
  ...
  // Likelihoods
  for (n in 1:N) {
    real eta_num = X[n] * beta_num + re[group_idx[n]] + phi_spatial[spatial_idx[n]];
    real eta_denom = X[n] * beta_denom + re[group_idx[n]] + phi_spatial[spatial_idx[n]];
    if (y_num[n] > 0) {
