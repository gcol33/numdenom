# =============================================================================
# Single-row benchmark runner with DENSE mass matrix
# Usage: Rscript benchmarks/bench_dense_row.R <row_number>
# Output: RESULT:<row>:<time_or_error>
#
# Identical to bench_single_row.R except metric = "dense"
# =============================================================================

args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 1) stop("Usage: Rscript bench_dense_row.R <row_number>")
ROW_NUM <- as.integer(args[1])

suppressPackageStartupMessages(library(numdenom))

# Standard parameters (same as bench_single_row.R for direct comparison)
N_OBS       <- 500L
N_ITER      <- 500L
N_WARMUP    <- 250L
N_CHAINS    <- 1L
...
# --- Data generation (identical to bench_single_row.R) ---
...
# --- Row configurations (identical to bench_single_row.R) ---
...
# --- Run benchmark with DENSE mass matrix ---
ratiod_args <- list(
  formula          = form,
  data             = df,
  family           = nd_family,
