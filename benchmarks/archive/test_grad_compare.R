...
fit_At <- tratio(
  ...
  control = list(iter = 3, warmup = 1, chains = 1, gradient_mode = "A_t", verbose = TRUE)
)
...
fit_H <- tratio(
  ...
  data = df,
  family = ratiod_lognormal(),
  control = list(iter = 3, warmup = 1, chains = 1, gradient_mode = "H", verbose = TRUE)
)
...
fit_At_long <- tratio(
  ...
  data = df,
  family = ratiod_lognormal(),
  control = list(iter = 100, warmup = 50, chains = 1, gradient_mode = "A_t", verbose = FALSE)
)
