...
test_that("plot_with_base produces trace plots", {
  ...
  expect_silent(
    suppressWarnings(
      ratiod:::plot_with_base(draws_array, type = "trace", n_chains = 1)
    )
  )
})
...
test_that("plot_with_base produces density plots", {
  ...
  expect_silent(
    suppressWarnings(
      ratiod:::plot_with_base(draws_array, type = "dens", n_chains = 1)
    )
  )
})
...
test_that("plot_with_base produces combined plots", {
  ...
  expect_silent(
    suppressWarnings(
      ratiod:::plot_with_base(draws_array, type = "both", n_chains = 1)
    )
  )
})
...
test_that("plot_with_base handles multi-chain", {
  ...
  expect_silent(
    suppressWarnings(
      ratiod:::plot_with_base(draws_array, type = "dens", n_chains = 2)
    )
  )
})
...
test_that("compute_fitted_values dispatches correctly", {
  ...
  result <- ratiod:::compute_fitted_values(fit)
...
test_that("compute_fitted_hmc works", {
  ...
  result <- ratiod:::compute_fitted_hmc(fit)
...
test_that("build_prediction_data works", {
  ...
  pred_data <- ratiod:::build_prediction_data(
...
test_that("compute_predictions_hmc works", {
  ...
  result <- ratiod:::compute_predictions_hmc(fit, pred_data, type = "response")
...
test_that("compute_predictions_hmc works with link scale", {
  ...
  result <- ratiod:::compute_predictions_hmc(fit, pred_data, type = "link")
