...
test_that("leapfrog preserves Hamiltonian for quadratic potential", {
  ...
  H_initial <- ratiod:::cpp_test_hamiltonian(q, p)

  # Small step size, many steps for accuracy
  result <- ratiod:::cpp_test_leapfrog(q, p, epsilon = 0.01, L = 100)

  H_final <- ratiod:::cpp_test_hamiltonian(result$q, result$p)
...
test_that("leapfrog is reversible", {
  ...
  # Forward integration
  fwd <- ratiod:::cpp_test_leapfrog(q, p, epsilon = 0.1, L = 10)

  # Backward integration (negate momentum, integrate, negate again)
  bwd <- ratiod:::cpp_test_leapfrog(fwd$q, -fwd$p, epsilon = 0.1, L = 10)
...
test_that("leapfrog with zero steps returns input",
{
  ...
  result <- ratiod:::cpp_test_leapfrog(q, p, epsilon = 0.01, L = 1)
...
test_that("Hamiltonian is sum of potential and kinetic energy", {
  ...
  H <- ratiod:::cpp_test_hamiltonian(q, p)
...
test_that("log_sum_exp is correct for simple cases", {
  ...
  result <- ratiod:::cpp_test_log_sum_exp(c(1, 2))
...
test_that("log_sum_exp handles large values without overflow", {
  ...
  result <- ratiod:::cpp_test_log_sum_exp(large_vals)
...
test_that("log_sum_exp handles negative values", {
  result <- ratiod:::cpp_test_log_sum_exp(c(-1000, -999, -998))
  expected <- -998 + log(exp(-2) + exp(-1) + 1)
  expect_equal(result, expected, tolerance = 1e-10)
})

test_that("log_sum_exp of single value returns that value", {
  expect_equal(ratiod:::cpp_test_log_sum_exp(5.0), 5.0)
})
...
test_that("softmax sums to 1", {
  x <- c(1, 2, 3, 4)
  result <- ratiod:::cpp_test_softmax(x)
...
test_that("softmax is shift-invariant", {
  x <- c(1, 2, 3)
  result1 <- ratiod:::cpp_test_softmax(x)
  result2 <- ratiod:::cpp_test_softmax(x + 1000)
...
test_that("inv_logit of 0 is 0.5", {
  result <- ratiod:::cpp_test_inv_logit(0)
...
test_that("inv_logit is bounded (0, 1)", {
  ...
  x <- c(-10, -5, -1, 0, 1, 5, 10)
  result <- ratiod:::cpp_test_inv_logit(x)
...
test_that("lgamma matches R's lgamma", {
  ...
  for (x in x_vals) {
    expect_equal(ratiod:::cpp_test_lgamma(x), lgamma(x), tolerance = 1e-10)
  }
})

test_that("lgamma(1) = 0", {
  expect_equal(ratiod:::cpp_test_lgamma(1), 0)
})

test_that("lgamma(n) = log((n-1)!) for integers", {
  expect_equal(ratiod:::cpp_test_lgamma(5), log(factorial(4)), tolerance = 1e-10)
  expect_equal(ratiod:::cpp_test_lgamma(10), log(factorial(9)), tolerance = 1e-10)
})
...
test_that("poisson_loglik matches dpois", {
  ...
  cpp_ll <- ratiod:::cpp_test_poisson_loglik(y, lambda)
...
test_that("poisson_loglik returns -Inf for impossible observations", {
  ...
  result <- ratiod:::cpp_test_poisson_loglik(y, lambda)
...
test_that("binomial_loglik matches dbinom", {
  ...
  cpp_ll <- ratiod:::cpp_test_binomial_loglik(y, n, p)
...
test_that("binomial_loglik handles edge probabilities", {
  ...
  expect_true(is.finite(ratiod:::cpp_test_binomial_loglik(
...
test_that("negbin_loglik matches dnbinom", {
  ...
  cpp_ll <- ratiod:::cpp_test_negbin_loglik(y, mu, phi)
...
test_that("normal_loglik matches dnorm", {
  ...
  cpp_ll <- ratiod:::cpp_test_normal_loglik(y, mu, sigma)
...
test_that("normal_loglik peaks at y = mu", {
  ...
  ll_at_mu <- ratiod:::cpp_test_normal_loglik(mu, mu, sigma)
  ll_away <- ratiod:::cpp_test_normal_loglik(mu + 1, mu, sigma)
...
test_that("matvec matches R's %*%", {
  ...
  cpp_result <- ratiod:::cpp_test_matvec(A, x)
...
test_that("matvec with identity matrix returns input", {
  ...
  result <- ratiod:::cpp_test_matvec(I, x)
...
test_that("matvec with zero matrix returns zeros", {
  ...
  result <- ratiod:::cpp_test_matvec(Z, x)
...
test_that("autodiff gradient of sum(x^2) is 2*x", {
  x <- c(1.0, 2.0, 3.0, -1.5)
  result <- ratiod:::cpp_test_autodiff_gradient(x)
...
test_that("autodiff chain rule works for exp(x^2)", {
  ...
  for (x in test_vals) {
    result <- ratiod:::cpp_test_autodiff_exp_chain(x)
...
test_that("autodiff Poisson log-likelihood gradient is correct", {
  ...
  result <- ratiod:::cpp_test_autodiff_log_likelihood(y, eta)
...
test_that("autodiff division gradients are correct", {
  ...
  for (tc in test_cases) {
    result <- ratiod:::cpp_test_autodiff_division(tc$a, tc$b)
...
test_that("autodiff lgamma gradient matches digamma", {
  ...
  for (x in test_vals) {
    result <- ratiod:::cpp_test_autodiff_lgamma(x)
...
test_that("autodiff softplus value and gradient are correct", {
  ...
  for (x in test_vals) {
    result <- ratiod:::cpp_test_autodiff_softplus(x)
...
test_that("autodiff softplus handles extreme values", {
  ...
  result_large <- ratiod:::cpp_test_autodiff_softplus(50.0)
  ...
  result_small <- ratiod:::cpp_test_autodiff_softplus(-50.0)
...
test_that("autodiff inv_logit value and gradient are correct", {
  ...
  for (x in test_vals) {
    result <- ratiod:::cpp_test_autodiff_inv_logit(x)
...
test_that("autodiff inv_logit(0) = 0.5 with correct gradient", {
  result <- ratiod:::cpp_test_autodiff_inv_logit(0.0)
...
test_that("autodiff log value and gradient are correct", {
  ...
  for (x in test_vals) {
    result <- ratiod:::cpp_test_autodiff_log(x)
...
test_that("autodiff sqrt value and gradient are correct", {
  ...
  for (x in test_vals) {
    result <- ratiod:::cpp_test_autodiff_sqrt(x)
...
test_that("autodiff pow value and gradient are correct", {
  ...
  for (tc in test_cases) {
    result <- ratiod:::cpp_test_autodiff_pow(tc$x, tc$p)
...
test_that("autodiff log1p value and gradient are correct", {
  ...
  for (x in test_vals) {
    result <- ratiod:::cpp_test_autodiff_log1p(x)
...
test_that("autodiff log_sum_exp value and gradients are correct", {
  ...
  for (tc in test_cases) {
    result <- ratiod:::cpp_test_autodiff_log_sum_exp(tc$a, tc$b)
...
test_that("autodiff logit value and gradient are correct", {
  ...
  for (x in test_vals) {
    result <- ratiod:::cpp_test_autodiff_logit(x)
...
test_that("autodiff negbin log-likelihood gradients are correct", {
  ...
  result <- ratiod:::cpp_test_autodiff_negbin_loglik(y, mu, phi)
  ...
  for (i in seq_along(mu)) {
    mu_plus <- mu
    mu_plus[i] <- mu[i] + eps
    result_plus <- ratiod:::cpp_test_autodiff_negbin_loglik(y, mu_plus, phi)

    mu_minus <- mu
    mu_minus[i] <- mu[i] - eps
    result_minus <- ratiod:::cpp_test_autodiff_negbin_loglik(y, mu_minus, phi)
...
test_that("cpp_rpg1 mean approximates E[PG(1,z)] = tanh(z/2)/(2z)", {
  ...
  z <- 1.0
  n <- 1000
  samples <- replicate(n, ratiod:::cpp_rpg1(z))
...
test_that("cpp_rpg with b=1 matches cpp_rpg1",
{
  set.seed(123)
  z <- c(0.5, 1.0, 2.0)
  result1 <- ratiod:::cpp_rpg(as.integer(rep(1, 3)), z)

  set.seed(123)
  result2 <- ratiod:::cpp_rpg1(z)
...
test_that("laplace binomial log-likelihood is correct", {
  ...
  for (tc in test_cases) {
    result <- ratiod:::cpp_test_laplace_binomial(tc$y, tc$n, tc$eta)
...
test_that("laplace binomial handles extreme eta values", {
  ...
  result_pos <- ratiod:::cpp_test_laplace_binomial(9L, 10L, 10.0)
  ...
  result_neg <- ratiod:::cpp_test_laplace_binomial(1L, 10L, -10.0)
...
test_that("laplace negbin log-likelihood is correct", {
  ...
  for (tc in test_cases) {
    result <- ratiod:::cpp_test_laplace_negbin(tc$y, tc$eta, tc$phi)
...
test_that("laplace negbin gradient matches finite differences", {
  ...
  result <- ratiod:::cpp_test_laplace_negbin(y, eta, phi)
  result_plus <- ratiod:::cpp_test_laplace_negbin(y, eta + eps, phi)
  result_minus <- ratiod:::cpp_test_laplace_negbin(y, eta - eps, phi)
...
test_that("laplace poisson log-likelihood is correct", {
  ...
  for (tc in test_cases) {
    result <- ratiod:::cpp_test_laplace_poisson(tc$y, tc$eta)
...
test_that("laplace poisson gradient matches finite differences", {
  ...
  result <- ratiod:::cpp_test_laplace_poisson(y, eta)
  result_plus <- ratiod:::cpp_test_laplace_poisson(y, eta + eps)
  result_minus <- ratiod:::cpp_test_laplace_poisson(y, eta - eps)
...
test_that("pg update_beta returns correct length", {
  ...
  result <- ratiod:::cpp_test_pg_update_beta(kappa, omega, X, re_contrib, prior_sd)
...
test_that("pg update_re returns correct length", {
  ...
  result <- ratiod:::cpp_test_pg_update_re(kappa, omega, X_beta, group, n_groups, sigma_re)
...
test_that("pg update_sigma_re returns positive value", {
  ...
  result <- ratiod:::cpp_test_pg_update_sigma_re(re, scale)
...
test_that("pg update_sigma_re is roughly proportional to RE standard deviation", {
  ...
  result_small <- ratiod:::cpp_test_pg_update_sigma_re(re_small, 2.5)

  # Large RE variance
  re_large <- rnorm(20, sd = 3.0)
  result_large <- ratiod:::cpp_test_pg_update_sigma_re(re_large, 2.5)
...
test_that("dot_product matches sum(x*y)", {
  ...
  result <- ratiod:::cpp_test_dot_product(x, y)
...
test_that("axpy computes y = a*x + y correctly", {
  ...
  result <- ratiod:::cpp_test_axpy(a, x, y)
  expected <- a * x + y
  expect_equal(result, expected, tolerance = 1e-10)
})
...
test_that("scale computes x = a*x correctly", {
  ...
  result <- ratiod:::cpp_test_scale(a, x)
...
test_that("linalg_matvec matches X %*% beta", {
  ...
  result <- ratiod:::cpp_test_linalg_matvec(X, beta)
...
test_that("linalg_matvec_add computes y + X*beta correctly", {
  ...
  result <- ratiod:::cpp_test_linalg_matvec_add(X, beta, y_init)
...
test_that("linalg_matvec_transpose matches t(X) %*% x", {
  ...
  result <- ratiod:::cpp_test_linalg_matvec_transpose(X, x)
...
col_idx <- c(1L, 0L, 2L, 1L, 3L, 2L)  # 0-based neighbors
  ...
  result <- ratiod:::cpp_test_sparse_laplacian_quadform(row_ptr, col_idx, x)
...
test_that("linalg_log_sum_exp is numerically stable", {
  # Normal case
  expect_equal(
    ratiod:::cpp_test_linalg_log_sum_exp(1.0, 2.0),
    log(exp(1) + exp(2)),
    tolerance = 1e-10
  )

  # Large values that would overflow
  result <- ratiod:::cpp_test_linalg_log_sum_exp(700, 701)
  expected <- 701 + log(exp(-1) + 1)
  expect_equal(result, expected, tolerance = 1e-10)

  # Negative values
  result <- ratiod:::cpp_test_linalg_log_sum_exp(-100, -99)
...
test_that("linalg_log_sum_exp_vec handles vectors", {
  x <- c(1, 2, 3)
  result <- ratiod:::cpp_test_linalg_log_sum_exp_vec(x)
  expected <- log(sum(exp(x)))
  expect_equal(result, expected, tolerance = 1e-10)

  # Large values
  x_large <- c(700, 701, 702)
  result_large <- ratiod:::cpp_test_linalg_log_sum_exp_vec(x_large)
...
test_that("softmax_inplace produces valid probabilities", {
  x <- c(1, 2, 3, 4)
  result <- ratiod:::cpp_test_softmax_inplace(x)
...
test_that("compute_linear_predictors works correctly", {
  ...
  result <- ratiod:::cpp_test_compute_linear_predictors(
...
test_that("rw1_quadratic_form computes sum of squared differences", {
  ...
  result <- ratiod:::cpp_test_rw1_quadratic_form(phi, FALSE)
...
test_that("rw1_quadratic_form handles cyclic case", {
  ...
  result <- ratiod:::cpp_test_rw1_quadratic_form(phi, TRUE)
...
test_that("rw2_quadratic_form computes sum of squared second differences", {
  ...
  result <- ratiod:::cpp_test_rw2_quadratic_form(phi, FALSE)
  d1 <- 1 - 2*3 + 4   # = -1
  d2 <- 3 - 2*4 + 6   # = 1
  d3 <- 4 - 2*6 + 10  # = 2
...
test_that("rw2_quadratic_form returns 0 for T < 3", {
  expect_equal(ratiod:::cpp_test_rw2_quadratic_form(c(1, 2), FALSE), 0)
  expect_equal(ratiod:::cpp_test_rw2_quadratic_form(c(1), FALSE), 0)
})
...
test_that("ar1_log_density is finite for valid inputs", {
  ...
  result <- ratiod:::cpp_test_ar1_log_density(phi, rho, tau)
...
test_that("temporal_log_prior RW1 matches manual computation", {
  ...
  result <- ratiod:::cpp_test_temporal_log_prior(phi, "rw1", tau, 0, FALSE)
...
test_that("sum_to_zero_penalty penalizes sum != 0", {
  ...
  result_centered <- ratiod:::cpp_test_sum_to_zero_penalty(phi_centered, 0.001)
  expect_equal(result_centered, 0.0, tolerance = 1e-10)

  # Non-centered phi
  phi_noncentered <- c(1, 2, 3)  # sum = 6
  result_noncentered <- ratiod:::cpp_test_sum_to_zero_penalty(phi_noncentered, 0.001)
...
test_that("log1pexp is numerically stable", {
  # Normal values
  expect_equal(ratiod:::cpp_test_log1pexp(0), log(2), tolerance = 1e-10)
  expect_equal(ratiod:::cpp_test_log1pexp(1), log(1 + exp(1)), tolerance = 1e-10)

  # Large positive (should return x)
  expect_equal(ratiod:::cpp_test_log1pexp(50), 50, tolerance = 1e-10)

  # Large negative (should return exp(x))
  expect_equal(ratiod:::cpp_test_log1pexp(-20), exp(-20), tolerance = 1e-15)
})

test_that("zi_logistic matches plogis", {
  test_vals <- c(-5, -1, 0, 1, 5)
  for (x in test_vals) {
    expect_equal(ratiod:::cpp_test_zi_logistic(x), plogis(x), tolerance = 1e-10)
  }
})

test_that("log_logistic is numerically stable", {
  # log(sigmoid(x))
  expect_equal(ratiod:::cpp_test_log_logistic(0), log(0.5), tolerance = 1e-10)

  # Large positive: log(sigmoid(x)) ≈ 0
  expect_equal(ratiod:::cpp_test_log_logistic(50), 0, tolerance = 1e-10)

  # Large negative: log(sigmoid(x)) ≈ x
  expect_equal(ratiod:::cpp_test_log_logistic(-50), -50, tolerance = 1e-10)
})

test_that("log1m_logistic is numerically stable", {
  # log(1 - sigmoid(x)) = log(sigmoid(-x))
  expect_equal(ratiod:::cpp_test_log1m_logistic(0), log(0.5), tolerance = 1e-10)

  # Large positive: log(1-sigmoid(x)) ≈ -x
  expect_equal(ratiod:::cpp_test_log1m_logistic(50), -50, tolerance = 1e-10)

  # Large negative: log(1-sigmoid(x)) ≈ 0
  expect_equal(ratiod:::cpp_test_log1m_logistic(-50), 0, tolerance = 1e-10)
})
...
test_that("zi_poisson_lpmf matches dpois", {
  ...
  for (tc in test_cases) {
    result <- ratiod:::cpp_test_zi_poisson_lpmf(tc$y, tc$mu)
...
test_that("zi_negbin_lpmf matches dnbinom", {
  ...
  for (tc in test_cases) {
    result <- ratiod:::cpp_test_zi_negbin_lpmf(tc$y, tc$mu, tc$phi)
...
test_that("zi_poisson_lpmf_logit handles y=0 correctly", {
  ...
  result <- ratiod:::cpp_test_zi_poisson_lpmf_logit(0L, mu, logit_zi)
...
test_that("zi_poisson_lpmf_logit handles y>0 correctly", {
  ...
  result <- ratiod:::cpp_test_zi_poisson_lpmf_logit(5L, mu, logit_zi)
...
test_that("truncated_poisson_lpmf is correct for y>0", {
  ...
  for (y in 1:10) {
    result <- ratiod:::cpp_test_truncated_poisson_lpmf(y, mu)
...
test_that("hurdle_poisson_lpmf_logit handles y=0 correctly", {
  ...
  result <- ratiod:::cpp_test_hurdle_poisson_lpmf_logit(0L, 3.0, logit_theta)
  # P(Y=0) = 1 - theta
...
test_that("hurdle_poisson_lpmf_logit handles y>0 correctly", {
  ...
  for (y in 1:5) {
    result <- ratiod:::cpp_test_hurdle_poisson_lpmf_logit(as.integer(y), mu, logit_theta)
...
test_that("zi_log_likelihood dispatches correctly", {
  ...
  result_none <- ratiod:::cpp_test_zi_log_likelihood(5L, mu, phi, logit_zi, "none")
  expected_none <- dnbinom(5, size = phi, mu = mu, log = TRUE)
  expect_equal(result_none, expected_none, tolerance = 1e-8)

  # ZI Poisson
  result_zip <- ratiod:::cpp_test_zi_log_likelihood(0L, mu, 0, logit_zi, "zi_poisson")
  expected_zip <- ratiod:::cpp_test_zi_poisson_lpmf_logit(0L, mu, logit_zi)
  expect_equal(result_zip, expected_zip, tolerance = 1e-10)

  # Hurdle Poisson
  result_hp <- ratiod:::cpp_test_zi_log_likelihood(3L, mu, 0, logit_zi, "hurdle_poisson")
  expected_hp <- ratiod:::cpp_test_hurdle_poisson_lpmf_logit(3L, mu, logit_zi)
...
test_that("zi_poisson_grad_logit_zi matches finite differences", {
  ...
  for (y in c(0L, 1L, 5L)) {
    grad <- ratiod:::cpp_test_zi_poisson_grad_logit_zi(y, mu, logit_zi)

    # Finite difference
    f_plus <- ratiod:::cpp_test_zi_poisson_lpmf_logit(y, mu, logit_zi + eps)
    f_minus <- ratiod:::cpp_test_zi_poisson_lpmf_logit(y, mu, logit_zi - eps)
...
test_that("hurdle_grad_logit_theta matches finite differences", {
  ...
  for (y in c(0L, 1L, 5L)) {
    grad <- ratiod:::cpp_test_hurdle_grad_logit_theta(y, logit_theta)

    # Finite difference
    f_plus <- ratiod:::cpp_test_hurdle_poisson_lpmf_logit(y, mu, logit_theta + eps)
    f_minus <- ratiod:::cpp_test_hurdle_poisson_lpmf_logit(y, mu, logit_theta - eps)
    fd_grad <- (f_plus - f_minus) / (2 * eps)
...
test_that("OpenMP max threads is at least 1", {
  max_threads <- ratiod:::cpp_test_get_max_threads()
...
test_that("parallel dot products produce correct results", {
  ...
  for (n_threads in c(1, 2, 4)) {
    result <- ratiod:::cpp_test_parallel_dot_products(X, y, n_threads)
...
test_that("parallel likelihood reduction is correct", {
  ...
  for (n_threads in c(1, 2, 4)) {
    result <- ratiod:::cpp_test_parallel_likelihood(y, mu, n_threads)
    expect_equal(result$log_lik, expected_ll, tolerance = 1e-8)
  }
})
...
test_that("parallel independent computations are thread-safe", {
  ...
  results_1t <- ratiod:::cpp_test_parallel_independent(n, 1L)

  for (n_threads in c(2, 4)) {
    results_mt <- ratiod:::cpp_test_parallel_independent(n, n_threads)
...
test_that("compute_linear_predictors is consistent across thread counts", {
  ...
  result_1t <- ratiod:::cpp_test_compute_linear_predictors(
    X_num, beta_num, X_denom, beta_denom, 1L
  )

  for (n_threads in c(2, 4)) {
    result_mt <- ratiod:::cpp_test_compute_linear_predictors(
